...
namespace tak2flac
{
   public class Program
   {
      ...
      // private static string FFMPEG_BINARY_PATH = "/usr/bin/ffmpeg";
        private static string FFPROBE_BINARY_PATH => GetEnv("FFPROBE_BINARY");
      // private static string FFPROBE_BINARY_PATH = "/usr/bin/ffprobe";
      private static bool RequireShortDuration = false;
      private static bool Verbose = false;
      private static bool DisableBatchMode = true;
      /// <summary>
      ...
      public static void Main(string[] args)
      {
         ...
         {
            output ??= "./";
            FFProbeMetadata? metadata = GetMediaMetadata(input);
            if(metadata == null)
            {
               Console.WriteLine("No found Metadata.");
               return ;
            }
            ...
            if (cueData == null)
            {
               Console.WriteLine("Cannot get CueData");
               return;
            }

            var Duration = TimeSpan.FromSeconds(metadata?.format?.duration ?? 0); // Im sorry I used wrong method to get full duration, use FromSeconds instead now.
            if (Verbose)
            ...
            if (!DisableBatchMode)
            {
               var param = CreateFFMpegParameters(output, input, cueData, Duration) 
                  ?? throw new NullReferenceException("param cant be null.");
               if (Verbose)
               {
            ...
            else
            {
               var parameters = CreateFFMpegParametersLines(output, cueData, Duration) 
                  ?? throw new NullReferenceException("parameters cant be null");
      ...
      public static string[]? CreateFFMpegParametersLines(string outputPath, string cueData, TimeSpan fullDuration)
      {
         var cueSheet = new Cuesheet(cueData) 
            ?? throw new NullReferenceException("cueSheet Cant be null");
         ...
         for (int i = 0; i < pointerCounts; i++)
         {
            ...
            {
               throw new NullReferenceException("start and end cant be null");
            }
            Timeline timeline = new((TimeSpan)start, (TimeSpan)end);
         ...
         foreach (var cueProbe in cueSheet.AudioProbes)
         {
            ...
            var result = $"-ss {timeline.Start:hh\\:mm\\:ss\\.ff} -t {timeline.Duration:hh\\:mm\\:ss\\.ff} {metadataParams} \"{outputPath + cueProbe.Title}.flac\" ";
            if (Verbose)
      ...
      static void CreateFFMpegProcess(string args, bool quietVerbose = true, FFMpegComponentEnum component = FFMpegComponentEnum.FFMPEG)
      {
         ...
         ProcessStartInfo arg = new($"\"{programPath}\"", (quietVerbose ? "-v panic " : "") + args)
         {
